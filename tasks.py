...
for item in actual_sheets:
    ...
    styled = grouped.style.set_properties(subset=["Total Qty"], **{'text-align': 'center'})
    styled["Price"] = styled["Price"].map('{:,d}'.format)
    destination = '/Users/detik/Project/extra/excel-pandas/Final Omzet.xlsx'
    
    WriteToExcel(destination, styled, item, False)
